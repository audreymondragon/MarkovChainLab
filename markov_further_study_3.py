...
def make_text(chains):
    ...
    #we want our start link to begin with a capital letter
    #sort our list of dictionary keys and only keep keys that start with capital letters
    #choose a random choice from this new pared down list
    #the rest of the code can then remain the same

    cap_tuples = []
    for key in chains.keys():
        if key[0][0] in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            cap_tuples.append(key)
            
    #getting a random tuple from our capital tuple list as a start link
    current_key = choice(cap_tuples)
    words = []
